...
int			overlap(t_point set[26][4], t_point ref[4], int current, t_point vect_it)
{
	int		i;
	int		j;
	int		k;
	...
	while (i < current)
	{
		...
		while (j < 4)
		{
			...
			while (k < 4)
			{
				if ((ref[j].y + vect_it.y == set[i][k].y) &&
					(ref[j].x + vect_it.x == set[i][k].x))
					return (1);
				k++;	
			}
			j++;
...
int			find_pos_for(t_etri data, t_point vect_it, t_point ref[4], t_point set[26][4])
{
...
void			find_solution(t_point ref[26][4], int tetri_nbr, t_point vect_it, t_etri data)
{									
	t_point		set[26][4];
